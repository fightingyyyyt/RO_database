...
class AppConfig:
    ...
    # ---------- 输出 ----------
    output_excel: str = "./test3/test4_output_regenerated.xlsx"
...
TRACEBACK_CONC_PATTERN = re.compile(
    r"(?P<value>[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\s*"    r"(?P<unit>%\s*\((?:w/v|m/v|w/w|v/v|wt)\)|wt\s*/?\s*v%?|w\s*/?\s*v%?|v\s*/?\s*v%?|"    r"vol\.?\s*%?|wt%|w/w%|m/m%|mass%|g\s*/\s*100\s*mL|g\s*/\s*L|mg\s*/\s*L|ug\s*/\s*L|ng\s*/\s*L|"    r"mg\s*/\s*mL|mmol\s*/\s*L|mol\s*/\s*L|mM|M|mL\s*/\s*L|ppm|ppb|g\s*/\s*kg|mg\s*/\s*kg|%)",
    flags=re.I
)
...
def recover_concentration_from_traceback(record: Dict[str, Any], snippets: List[str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "reason": f"Recovered from MD snippet by regex proximity search (keyword='{best['keyword'] or 'N/A'}', score={best['score']}).",
    ...
    for idx, row in main_df.iterrows():
        ...
        for slot_spec in SLOT_SPECS:
            ...
            for item in items:
                ...
                elif missing_concentration_for_traceback:
                    recovered = recover_concentration_from_traceback(rec, snippets)
                    rec["traceback_candidate_count"] = recovered.get("candidate_count", 0)
                    rec["traceback_candidate_reason"] = recovered.get("reason", "")
                    rec["traceback_candidate_snippet"] = recovered.get("snippet", "")

                    if recovered.get("found"):
                        rec["traceback_candidate_value"] = recovered.get("value", "")
                        rec["traceback_candidate_unit"] = recovered.get("unit", "")
                        rec["traceback_candidate_canonical_unit"] = recovered.get("canonical_unit", "")
                        rec["resolved_value_for_conversion"] = recovered.get("value", "")
                        rec["resolved_unit_for_conversion"] = recovered.get("unit", "")
                        ...
                        if not rec_for_conversion.get("percent_type_inferred") and rec_for_conversion["unit_family"] == "percent_ambiguous":
                            rec_for_conversion["percent_type_inferred"] = judge.get("percent_type_inferred", "")
                        ...
                        reason_parts = [
                            f"Missing concentration in structured field; recovered candidate from MD: {rec['resolved_value_for_conversion']} {rec['resolved_unit_for_conversion']}",
                        ...
                        reason_parts = [
                            f"Solute present but concentration missing in structured field ({parse_error}).",
                            recovered.get("reason", ""),
