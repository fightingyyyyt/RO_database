...
class AppConfig:
    ...
    # ---------- 输出 ----------
    output_excel: str = "./test3/test2_output.xlsx"
...
ABBREVIATION_EXACT_ONLY_MAXLEN = 6
COMMON_AQUEOUS_PHASE_SOLUTES = {
    "nacl", "sodium chloride", "mpd", "m-phenylenediamine", "piperazine",
    "sds", "sodium dodecyl sulfate", "edc", "nhs", "triethylamine", "tea",
    "sodium hydroxide", "naoh", "sodium carbonate", "camphorsulfonic acid", "csa",
    "cucl2", "ascorbic acid"
}
...
def _looks_like_short_abbreviation(s: str) -> bool:
    raw = norm_text(s)
    if not raw:
        return False
    raw_compact = re.sub(r"[\s\-_/\\]+", "", raw)
    return bool(re.fullmatch(r"[A-Za-z]{1,6}[A-Za-z0-9]*", raw_compact)) and len(raw_compact) <= ABBREVIATION_EXACT_ONLY_MAXLEN


def _allow_simplified_alias(s: str) -> bool:
    raw = norm_text(s)
    simp = simplify_key(raw)
    if not raw or not simp:
        return False
    if _looks_like_short_abbreviation(raw):
        return False
    if len(simp) >= 7:
        return True
...
def normalize_unit(raw_unit: Any) -> Dict[str, str]:
    ...
    if (
        s_nospace in {"wt%", "wt.%", "weight%", "mass%", "wtpercent", "masspercent", "w/w%", "%w/w", "m/m%", "%m/m", "weightpercent"}
        or "bymass" in s_nospace
        or "w/w" in s_nospace
        or "m/m" in s_nospace
    ):
    ...
    if (
        s_nospace in {"w/v%", "%w/v", "wt/v%", "%wt/v", "w/v", "wt/v", "wv", "wtv", "g/100ml", "g/100milliliter", "g/100milliliters", "g/dl"}
        or re.fullmatch(r"g/100m?l", s_nospace)
    ):
        canon = "g/100mL" if s_nospace in {"g/100ml", "g/dl"} or re.fullmatch(r"g/100m?l", s_nospace) else "w/v%"
    ...
    mass_conc_map = {
        ...
        "mg/ml": "mg/mL", "mgml-1": "mg/mL",
    }
    if s_nospace in mass_conc_map:
        return {"original_unit": raw, "canonical_unit": mass_conc_map[s_nospace], "unit_family": "mass_concentration"}
    ...
    if s_nospace in molarity_map:
        return {"original_unit": raw, "canonical_unit": molarity_map[s_nospace], "unit_family": "molarity"}

    if s_nospace in {"v/v%", "%v/v", "v/v", "vol%", "vol.%", "vol", "ml/l"}:
        canon = "mL/L" if s_nospace == "ml/l" else ("vol%" if s_nospace in {"vol%", "vol.%", "vol"} else "v/v%")
        return {"original_unit": raw, "canonical_unit": canon, "unit_family": "volume_fraction"}

    if s_nospace in {"ppm", "ppb", "g/kg", "mg/kg"}:
        return {"original_unit": raw, "canonical_unit": s_nospace if s_nospace in {"g/kg", "mg/kg"} else s_nospace, "unit_family": "ppm_family"}

    if re.fullmatch(r"\d+\s*:\s*\d+", raw.strip()):
        return {"original_unit": raw, "canonical_unit": raw, "unit_family": "unsupported"}

    if s_nospace in {"drops", "drop", "g", "ml", "mmol", "mol", "l", "ul"}:
        return {"original_unit": raw, "canonical_unit": raw, "unit_family": "unsupported"}
...
def split_slot_items(row: pd.Series, slot_spec: SlotSpec) -> List[Dict[str, Any]]:
    ...
    def align(lst: List[str], fill: str = "") -> List[str]:
        if len(lst) == 0:
            return [fill] * n
        if len(lst) == 1 and n > 1:
            return lst * n
        if len(lst) < n:
            warnings.append(f"padded_from_{len(lst)}_to_{n}")
            return lst + [fill] * (n - len(lst))
        return lst[:n]

    solutes_e = align(solutes, "NaCl" if slot_spec.slot_name == "test_nacl" else "")
    values_e = align(values, "")
    units_e = align(units, "")
    ...
    for i in range(n):
        v, u = split_inline_value_and_unit(values_e[i], units_e[i])
        parse_error = ""
        if not norm_text(v) and not norm_text(u):
    ...
    for _, r in identity_df.iterrows():
        ...
        for key in [original_name, standardized_solute, iupac, formula]:
            ...
            if nk and nk not in lookup:
                lookup[nk] = rec.copy()
            if _allow_simplified_alias(key):
                sk = simplify_key(key)
                if sk and sk not in simplify_lookup:
                    simplify_lookup[sk] = rec.copy()

    lookup["__simplify_lookup__"] = simplify_lookup
    return lookup
...
def resolve_solute_identity(original_solute: str, identity_lookup: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if nk in identity_lookup:
        return identity_lookup[nk].copy()

    if not _looks_like_short_abbreviation(raw):
        simp_lookup = identity_lookup.get("__simplify_lookup__", {})
        sk = simplify_key(raw)
        if sk in simp_lookup:
            return simp_lookup[sk].copy()
...
def fetch_molecular_weight(record: Dict[str, Any], cfg: AppConfig, pubchem_cache: Dict[str, Any]) -> Tuple[Optional[float], str]:
    ...
    if mw is None:
        for name in unique_keep_order([iupac, std_name, original_solute]):
            if not name:
                continue
            if name != iupac and _looks_like_short_abbreviation(name):
                continue
            try:
                mw = query_pubchem_mw_by_name(name, timeout_seconds=cfg.timeout_seconds)
                if mw is not None:
...
def infer_slot_phase(row: pd.Series, slot_spec: SlotSpec, record: Dict[str, Any], solvent_identified: str) -> str:
    if slot_spec.default_phase in {"aqueous", "organic"}:
        return slot_spec.default_phase
...
def build_unit_catalog(unit_df: Optional[pd.DataFrame], review_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if unit_df is not None and not unit_df.empty:
        ...
        for unit in review_df.get("original_unit", pd.Series(dtype=str)).fillna("").astype(str):
...
def build_review_sheet(
    ...
    for idx, row in main_df.iterrows():
        ...
        for slot_spec in SLOT_SPECS:
            ...
            for item in items:
                ...
                solvent_identified = infer_solvent_from_row(row, slot_spec)
                rec["solvent_identified"] = solvent_identified
                rec["phase"] = infer_slot_phase(row, slot_spec, rec, solvent_identified)
                ...
                if rec["split_parse_error"] == "missing_value_and_unit":
                    rec["suggested_wtpercent"] = None
                    rec["suggested_status"] = "FAILED_PARSE"
                    rec["suggested_reason"] = "Sub-item exists but value/unit both missing after alignment."
                else:
                    conv = convert_to_wtpercent(rec, density_info, ndigits=cfg.round_digits)
                    rec["suggested_wtpercent"] = conv["suggested_wtpercent"]
                    rec["suggested_status"] = conv["suggested_status"]
                    reason = conv["suggested_reason"]
                    if rec.get("split_parse_warning"):
                        reason = f"{reason} | split_warning={rec['split_parse_warning']}"
                    rec["suggested_reason"] = reason
    ...
    review_df = pd.DataFrame(review_rows)
    desired_cols = [
        "row_id", "slot_name", "item_index", "original_solute", "standardized_solute", "IUPAC_Name",
        "formula", "CID", "original_value", "original_unit", "canonical_unit", "unit_family", "phase",
        "solvent_identified", "physical_form_inferred", "percent_type_inferred", "MW", "MW_source",
        "density_source", "suggested_wtpercent", "suggested_status", "suggested_reason", "final_value",
        "final_unit", "final_status", "final_reason", "identity_source", "judgement_source",
        "judgement_reason", "need_traceback_flag", "traceback_snippets", "split_parse_error",
        "split_parse_warning", "MW_from_table",
    ]
    cols = [c for c in desired_cols if c in review_df.columns] + [c for c in review_df.columns if c not in desired_cols]
