...
# -----------------------------
# LLM prompt (DeepSeek JSON mode)
# -----------------------------
def build_llm_system_prompt() -> str:
    # 注意：DeepSeek JSON mode 要求提示里出现 “json”
...
return a clean structured classification for downstream PubChem lookup ONLY when appropriate.
...
def llm_clean_batch(
    ...
    """
    DeepSeek 用 chat.completions + response_format json_object
    """
...
def main():
    ...
    parser.add_argument("--input", default="./test4_LLM/step1.xlsx", help="Input Excel file path (default: ./test4_LLM/step1.xlsx)")
    ...
    parser.add_argument("--outdir", default="out_deepseek", help="Output directory (default: out_deepseek)")
    ...
    pubchem_cache = load_jsonl_cache(pubchem_cache_path)
       ...
       # Load Excel  ✅（必须在 def main(): 里面）
    df_obj = pd.read_excel(args.input, sheet_name=args.sheet)

    # 如果读出来是 dict（多 sheet），默认取第一个 sheet
    if isinstance(df_obj, dict):
        if len(df_obj) == 0:
            raise ValueError("Excel 没有任何 sheet。")
    ...
    print("[Debug] df type:", type(df), "shape:", df.shape)
    ...
    if df.shape[1] == 0:
        ...
        print(f"[DeepSeek LLM] total unique: {len(unique_names)} | cached: {len(results_llm)} | to_run: {len(todo)}")
