...
DEFAULT_RAW_SHEET = ""  # 1原始库为 xlsx 时的 sheet 名；空字符串表示自动选择
...
DEFAULT_OUTPUT_PATH = "./test12/output/test9_index_backtrace_pubchem.xlsx"  # 输出 Excel 路径
DEFAULT_REPORT_PATH = "./test12/output/test9_index_backtrace_pubchem_report.json"  # 输出 JSON 报告路径（可选，不填则不生成）
...
# 即使 PubChem 没命中，以下也属于"有价值的回溯结果"
BACKTRACE_POSITIVE_STATUSES = {
    "BACKTRACE_OK",
    "BACKTRACE_CLASSIFIED",
    "BACKTRACE_ENRICHED",
}
...
TOO_GENERIC_SINGLE_WORDS = {
    "membrane", "membranes", "glass", "water", "support", "substrate",
    "gold", "silver", "copper", "iron", "zinc", "nickel", "platinum",
    "acid", "base", "salt", "solvent", "oil", "gas", "air",
    "solution", "mixture", "sample", "surface", "coating", "layer",
    "film", "fiber", "fibre", "tube", "rod", "wire", "bead",
    "particle", "powder", "crystal", "liquid", "solid", "gel",
    "additive", "filler", "dopant", "catalyst", "reagent", "monomer",
    "surfactant", "plasticizer", "stabilizer", "initiator",
    "sand", "clay", "carbon", "nitrogen", "oxygen", "hydrogen",
    "rubber", "plastic", "metal", "ceramic", "concrete", "wood",
    "paper", "fabric", "foam", "sponge", "mesh", "grid",
}

PROPERTY_PHENOMENON_WORDS = {
    "hydrophilicity", "hydrophobicity", "roughness", "permeability",
    "wettability", "porosity", "viscosity", "conductivity", "resistance",
    "selectivity", "rejection", "fouling", "antifouling", "anti-fouling",
    "crosslinking", "cross-linking", "degradation", "aging", "ageing",
    "stability", "durability", "flexibility", "rigidity", "elasticity",
    "adsorption", "absorption", "desorption", "diffusion", "osmosis",
    "compaction", "swelling", "shrinkage", "corrosion", "erosion",
    "turbidity", "hardness", "softness", "brightness", "opacity",
    "transparency", "adhesion", "cohesion", "friction", "abrasion",
    "flux", "recovery", "retention", "leaching", "bleeding",
    "crystallinity", "amorphousness", "morphology", "topology",
    "zeta potential", "contact angle", "surface energy",
}

PROCESS_METHOD_WORDS = {
    "filtration", "distillation", "evaporation", "condensation",
    "precipitation", "centrifugation", "sonication", "ultrasonication",
    "extrusion", "casting", "spinning", "electrospinning", "coating",
    "dipping", "spraying", "sputtering", "annealing", "sintering",
    "calcination", "pyrolysis", "polymerization", "copolymerization",
    "grafting", "blending", "mixing", "stirring", "heating", "cooling",
    "drying", "washing", "rinsing", "etching", "oxidation", "reduction",
    "functionalization", "modification", "treatment", "preparation",
    "fabrication", "synthesis", "characterization", "measurement",
    "analysis", "testing", "evaluation", "optimization", "comparison",
}
...
def _is_too_generic_for_backtrace(name: str) -> bool:
    """判断一个名称是否太泛、太普通，不值得回原文查。"""
    nl = name.lower().strip()
    if not nl:
        return True
    ...
    words = nl.split()
    if len(words) == 1:
        bare = re.sub(r"[^a-z]", "", nl)
        if bare in TOO_GENERIC_SINGLE_WORDS:
            return True
        if bare in PROPERTY_PHENOMENON_WORDS:
            return True
        if bare in PROCESS_METHOD_WORDS:
            return True
        if len(bare) <= 3 and not re.fullmatch(r"[A-Z]{2,3}", name.strip()):
            return True

    if nl in PROPERTY_PHENOMENON_WORDS or nl in PROCESS_METHOD_WORDS:
        return True
    if re.sub(r"[-\s]", "", nl) in {re.sub(r"[-\s]", "", w) for w in PROPERTY_PHENOMENON_WORDS}:
        return True

    if len(words) <= 2:
        if all(re.sub(r"[^a-z]", "", w) in TOO_GENERIC_SINGLE_WORDS
               for w in words if len(w) >= 2):
            return True

    if re.fullmatch(r"(the|a|an|this|these|some|other|various|several)\s+\w+", nl):
        return True

    return False


def classify_backtrace_mode(row: pd.Series) -> str:
    """
    判断一行是否值得回原文核查，以及走哪种回溯模式。

    返回值:
      "abbreviation"           缩写/代号类 → 优先找定义句
      "general_unresolved"     NO_HIT 的非缩写 → 找更规范写法
      "material_or_commercial" 材料/商业类 → 找组成/类别信息
      ""                       不需要回溯（太泛 / 非化学实体 / 已解析）
    """
    ...
    std_q = normalize_text(row.get("Standardized_Query", ""))

    if looks_like_abbreviation(name):
        return "abbreviation"

    if _is_too_generic_for_backtrace(name):
        return ""

    if status not in DEFAULT_UNRESOLVED and status != "":
        return ""

    nl = name.lower()

    if detect_material_from_text(name, std_q):
        return "material_or_commercial"
    if any(k in nl for k in [
        "thin film", "reverse osmosis", "nanofiltration",
        "commercial", "hollow fiber", "flat sheet",
    ]):
        return "material_or_commercial"
    if status in {"NO_SPECIFIC_CHEMICAL", "NOT_CHEMICAL_FROM_LLM"}:
        return "material_or_commercial"

    if status in {"NO_HIT", "ABBR_EXPANDED_NO_HIT", "EMPTY_QUERY", "EMPTY_CANDIDATE", "CID_ONLY", ""}:
        if len(name) >= 6 and len(name.split()) >= 2:
            return "general_unresolved"
        if len(name) >= 4 and re.search(r"[-()/]", name):
            return "general_unresolved"
...
def cleanup_candidate_name(cand: str) -> str:
    ...
    c = c.strip(" ,;:-")
    c = re.sub(r"^.*?\b(namely|is|was|were|denoted as|abbreviated as|stands for|means)\b\s*", "", c, flags=re.I)
    return c.strip(" ,;:-")
...
def is_bad_candidate(cand: str) -> bool:
    ...
    if re.fullmatch(r"[A-Z0-9\-_/]+", cand):
        return True
    return any(p in c for p in BAD_CANDIDATE_PHRASES)
...
def init_bt_result(row: pd.Series, matched_source_record_index: Any = "", backtrace_mode: str = "") -> Dict[str, Any]:
    return {
...
def rank_status(status: str) -> int:
    order = {
        "BACKTRACE_OK": 7,
        "BACKTRACE_CLASSIFIED": 6,
        "BACKTRACE_ENRICHED": 5,
...
def process_one_source(
    ...
    model: str,
    enable_llm: bool,
    backtrace_mode: str = "abbreviation",
) -> Dict[str, Any]:
    original_name = normalize_text(row.get("Original_Name", ""))
    standardized_query = normalize_text(row.get("Standardized_Query", ""))

    best = init_bt_result(row, source_idx, backtrace_mode=backtrace_mode)
    ...
    def _is_enriched_result(res: Dict[str, Any]) -> bool:
        """对于非缩写模式，判断结果是否包含有价值的补充信息（即使没 CID）。
        标准较严格：仅有 Evidence_Snippet 不算；需要可信的全称或组成。"""
        full = normalize_text(res.get("Resolved_Full_Name", ""))
        comp = normalize_text(res.get("Resolved_Composition", ""))
        if full and len(full) >= 4 and not is_bad_candidate(full):
            fl = full.lower()
            if fl not in TOO_GENERIC_SINGLE_WORDS and fl not in PROPERTY_PHENOMENON_WORDS:
                return True
        if comp and len(comp) >= 6 and " | " in comp:
            return True
        return False
    ...
    for member, locate_method in candidates:
        ...
        for rc in ranked[:DEFAULT_MAX_REGEX_CANDIDATES]:
            ...
            current = init_bt_result(row, source_idx, backtrace_mode=backtrace_mode)
            current.update({
            ...
            if chem.get("Lookup_Status") in {"Polymer", "NO_SPECIFIC_CHEMICAL"}:
                current["Backtrace_Status"] = "BACKTRACE_CLASSIFIED"
                return current
            if backtrace_mode != "abbreviation" and _is_enriched_result(current):
                current["Backtrace_Status"] = "BACKTRACE_ENRICHED"
                return current
        ...
        if best["Backtrace_Status"] == "NOT_PROCESSED":
            ...
            best["Backtrace_Notes"] = merge_notes(
                f"mode={backtrace_mode}", f"md_locate={locate_method}",
...
def process_one_row(
    ...
    if not source_idxs:
        best = init_bt_result(row, "", backtrace_mode=backtrace_mode)
    ...
    for source_idx in tried:
        ...
        if best is None or rank_status(current.get("Backtrace_Status", "")) > rank_status(best.get("Backtrace_Status", "")):
            best = current
        if current.get("Backtrace_Status") in BACKTRACE_POSITIVE_STATUSES:
            break
...
def merge_backtrace_to_main(entity_df: pd.DataFrame, bt_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Backtrace_Status" in merged.columns:
        bt_status = merged["Backtrace_Status"].astype(str)
        hit_mask = bt_status.eq("BACKTRACE_OK")
        class_mask = bt_status.eq("BACKTRACE_CLASSIFIED")
        enriched_mask = bt_status.eq("BACKTRACE_ENRICHED")
    else:
        hit_mask = pd.Series(False, index=merged.index)
        class_mask = pd.Series(False, index=merged.index)
        enriched_mask = pd.Series(False, index=merged.index)

    chem_overwrite_mask = hit_mask | class_mask
    for dest, src in overwrite_map.items():
        if dest == "Standardized_Query":
            continue
        if src in merged.columns and dest in merged.columns and chem_overwrite_mask.any():
            merged[dest] = merged[dest].astype("object")
            merged.loc[chem_overwrite_mask, dest] = merged.loc[chem_overwrite_mask, src].fillna("").astype("object")
    ...
    def _is_trustworthy_for_std_query(x: Any) -> bool:
        ...
        if re.match(r"^(and|or|but|also|then|so|yet|nor|for|as|if)\b", sl):
            return False
        if re.match(r"^(reported|used|prepared|obtained|shown|found|"
                     r"described|observed|noted|demonstrated|known|"
                     r"referred|called|named|made|formed|given)\b", sl):
            return False
        ...
        if re.search(r"\b\d+(\.\d+)?\s*(%|wt|vol|mol|mg|g/l|ml|min|hour|h|ppm|ppb|bar|mpa|kpa)\b", sl, re.I):
            return False
        if re.search(r"\b(fig|figure|table|eq|equ|scheme|ref|section|chapter)\s*\.?\s*\d", sl, re.I):
            return False
        ...
        if re.search(r"\b(higher|lower|better|worse|increased|decreased|"
                      r"improved|enhanced|reduced|more|less|most|least)\s+\w+\s+\w+", sl):
            return False
    ...
    if "Standardized_Query" in merged.columns:
        any_positive = hit_mask | class_mask | enriched_mask
        ...
        bt_q_ok = bt_q.map(_is_trustworthy_for_std_query) & any_positive
        res_ok = res_full.map(_is_trustworthy_for_std_query) & any_positive
    ...
    if "Lookup_Status" in merged.columns:
        ...
        if enriched_mask.any():
            merged.loc[enriched_mask, "Lookup_Status"] = "BACKTRACE_ENRICHED"
    ...
    if "Match_Source" in merged.columns:
        ...
        if enriched_mask.any():
            merged.loc[enriched_mask, "Match_Source"] = "MD Backtrace (enriched)"
    ...
    if "Decision_Notes" in merged.columns:
        ...
        notes_mask = hit_mask | class_mask | enriched_mask
        if notes_mask.any():
...
def main() -> None:
    ...
    if args.process_all_unresolved:
        work_df["_bt_mode"] = "all"
    else:
        work_df["_bt_mode"] = work_df.apply(classify_backtrace_mode, axis=1)
        work_df = work_df[work_df["_bt_mode"].ne("")].copy()
    mode_counts = work_df["_bt_mode"].value_counts().to_dict()
    stage(f"回溯模式分布: {mode_counts}")
    ...
    for i, (_, row) in enumerate(iterator, start=1):
        ...
        bt = process_one_row(
            ...
            backtrace_mode=row.get("_bt_mode", "abbreviation"),
        )
        bt_rows.append(bt)
    ...
    summary = {
        ...
        "backtrace_mode_counts": mode_counts,
        "backtrace_status_counts": bt_df["Backtrace_Status"].value_counts(dropna=False).to_dict() if len(bt_df) else {},
        ...
        "enriched_hits": int((bt_df["Backtrace_Status"] == "BACKTRACE_ENRICHED").sum()) if len(bt_df) else 0,
