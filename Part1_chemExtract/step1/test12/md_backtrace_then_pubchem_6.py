...
# =========================
# 配置区（保留在前面，便于直接改） 对比第五版 不再只盯缩写 加强回溯质量 缩写和非缩写都能走“候选对齐检查”
# =========================
...
DEFAULT_OUTPUT_PATH = "./test12/output/test6_1.xlsx"  # 输出 Excel 路径
DEFAULT_REPORT_PATH = "./test12/output/test6_1.json"  # 输出 JSON 报告路径（可选，不填则不生成）

DEFAULT_BT_INPUT_PATH = r"D:\graduate_work\RO_database\Part1_chemExtract\step1\test12\output\test6_1_bt_only.xlsx"  # 仅合并模式：已有 bt_only.xlsx 的路径
...
DEFAULT_ENABLE_PREWRITE_ALIGNMENT = True
...
def normalize_pubchem_query(query: Any) -> str:
    ...
    q = re.sub(r"^(ed|ing|ly)\s+", "", q, flags=re.I)
    q = re.sub(r"^(solution of|aqueous solution of|mixture of|blend of|blend containing|containing|including|comprised of|consisting of|composed of|derived from)\s+", "", q, flags=re.I)
    q = re.sub(r"^(the|a|an|this|these|those)\s+", "", q, flags=re.I)
    q = re.sub(r"\s*[,;:]\s*$", "", q)
    q = q.strip(' ,;:-')
    if re.search(r"\b(and|or)\b", q, flags=re.I) and ',' in q:
        q = q.split(',')[0].strip()
    if q.lower().startswith('of '):
        q = q[3:].strip()
...
def is_trustworthy_std_query(x: Any) -> bool:
    ...
    if re.fullmatch(r"[A-Z][a-z]+", s):
        return False
    if sl in GENERIC_SINGLE_WORDS:
        return False
    if looks_like_nonchemical_generic(sl):
        return False
...
def align_candidate_for_pubchem(token: str, candidate: str, context_windows: List[str], base_mod, api_key: str, model: str, strict_abbr: bool = False) -> Dict[str, Any]:
    ...
    out = {
        ...
        "entity_type": "SPECIFIC_CHEMICAL" if trusted else "NO_CLUE",
        "reason": "rule_gate_pass" if trusted else "rule_gate_fail",
        "retry_backtrace": not trusted,
        "confidence": 0.55 if trusted else 0.0,
    }
    if api_key:
        llm = call_alignment_guard_llm(token, raw or candidate, context_windows, base_mod, api_key, model)
        if llm:
            clean = normalize_pubchem_query(llm.get("clean_name", raw))
            llm_ok = llm.get("is_valid_entity", False) and is_trustworthy_std_query(clean)
            if strict_abbr:
                llm_ok = llm_ok and is_trusted_abbreviation_expansion(clean, token)
            out = {
                "accepted": llm_ok,
                "clean_name": clean if llm_ok else "",
                "entity_type": llm.get("entity_type", "NO_CLUE"),
                "reason": llm.get("reason", ""),
                "retry_backtrace": bool(llm.get("retry_backtrace", False) or not llm_ok),
...
def try_lookup_query(
    ...
    query = normalize_pubchem_query(query)
    ...
    for member, locate_method in candidates:
                ...
                if llm_exp and llm_exp.get("entity_type", "NO_CLUE") != "NO_CLUE":
                    ...
                    if exp_full and exp_type == "SPECIFIC_CHEMICAL":
                        aligned = align_candidate_for_pubchem(
                            ...
                            base_mod=base_mod,
                            api_key=api_key if DEFAULT_ENABLE_PREWRITE_ALIGNMENT else "",
                            model=model,
                            strict_abbr=True,
                        )
        ...
        for rc in ranked[:DEFAULT_MAX_REGEX_CANDIDATES]:
            ...
            aligned = align_candidate_for_pubchem(
                ...
                context_windows=windows,
                base_mod=base_mod,
                api_key=api_key if (enable_llm and DEFAULT_ENABLE_PREWRITE_ALIGNMENT) else "",
                model=model,
                strict_abbr=is_abbr,
            )
...
def merge_backtrace_to_main(entity_df: pd.DataFrame, bt_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Standardized_Query" in merged.columns:
        ...
        bt_q_ok = bt_q.map(is_trustworthy_std_query) & hit_mask
        res_ok = res_full.map(is_trustworthy_std_query) & hit_mask
    ...
    for col in out.columns:
        if out[col].dtype == object:
            out[col] = out[col].apply(
                lambda v: _ILLEGAL_CHARS_RE.sub("", str(v)) if pd.notna(v) and isinstance(v, str) else v
            )
    return out
...
def main() -> None:
    ...
    if args.enable_llm and not normalize_text(args.api_key):
        stage("提醒: 你启用了 LLM，但没有提供 DEEPSEEK_API_KEY / --api-key。")
        stage("提醒: 本次将自动关闭 LLM，回溯仅使用规则与 PubChem。")
