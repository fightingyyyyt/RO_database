...
DEFAULT_OUTPUT_PATH = "./test12/output/test8_3.xlsx"  # 输出 Excel 路径
DEFAULT_REPORT_PATH = "./test12/output/test8_3.json"  # 输出 JSON 报告路径（可选，不填则不生成）
...
# ---------------------------------------------------------------------------
# KNOWN_ABBR_MAP: domain-specific abbreviation → canonical PubChem query
# Used BEFORE calling DeepSeek/PubChem to rescue high-value small-molecule hits.
# Keys are upper-case canonical abbreviations; values are PubChem-friendly names.
# ---------------------------------------------------------------------------
KNOWN_ABBR_MAP: Dict[str, str] = {
    ...
    "BHPD":  "1,3-bis(4-aminophenoxy)propane",
    "DABA":  "3,5-diaminobenzoic acid",
    "TAPA":  "1,3,5-tris(4-aminophenyl)benzene",
    ...
    "DMMPD": "2,4-dimethyl-m-phenylenediamine",
    # ── Solvents ─────────────────────────────────────────────────────────────
    ...
    # ── Functional reagents / coupling agents ────────────────────────────────
    ...
    # ── Membrane-domain foulants / model solutes ──────────────────────────────
    ...
    "BPS":   "bisphenol S",
    "BPF":   "bisphenol F",
    # ── Heavy metals (common ionic forms) ─────────────────────────────────────
    "Pb":    "lead",
    "Cd":    "cadmium",
    "Cu":    "copper",
    "Cr":    "chromium",
...
def normalize_pubchem_query(query: Any) -> str:
    ...
    # ── 新增：去掉用途/来源/条件残片 ──
    # 去掉 "purchased from / obtained from / supplied by" 及其后内容
    q = re.sub(r"\b(purchased|obtained|supplied|provided|sourced|acquired)\s+(from|by)\b.*$", "", q, flags=re.I)
    # 去掉纯度描述
    q = re.sub(r"\b(with|of)\s+(?:high\s+)?purity\b.*$", "", q, flags=re.I)
    q = re.sub(r"\s*[≥≤><=]\s*\d+(?:\.\d+)?\s*%.*$", "", q)
    # 去掉规格词（anhydrous / reagent grade 等）
    q = re.sub(r"\b(anhydrous|reagent\s+grade|analytical\s+grade|technical\s+grade|hplc\s+grade|acs\s+grade|laboratory\s+grade)\b.*$", "", q, flags=re.I)
    # 去掉处理描述（stored over / dried over 等）
    q = re.sub(r"\b(stored|dried|distilled|purified|filtered|degassed)\s+(over|from|under|with)\b.*$", "", q, flags=re.I)
    # 去掉 LaTeX 数学片段（如 $99.8\%$）
    q = re.sub(r"\$[^$]*\$", "", q)
    # 去掉 "#" 及其后内容（章节标记）
    q = re.sub(r"\s*#.*$", "", q)
    # 去掉括号内纯数字/百分比（如 (99%)）
    q = re.sub(r"\(\s*\d+(?:\.\d+)?\s*%?\s*\)", "", q)
    q = re.sub(r"\s*[,;:]\s*$", "", q)
    q = q.strip(" ,;:-")
...
# ---------------------------------------------------------------------------
# Helpers to boost hit rate: core-name extraction + PubChem-worth gating
# ---------------------------------------------------------------------------

# Shell words that wrap a core chemical name but carry no CID-value themselves.
_CORE_STRIP_PREFIXES = re.compile(
    r"^(?:"
    r"(?:aqueous|dilute|concentrated|saturated|anhydrous|dry|pure|fresh|degassed|filtered)\s+"
    r"|(?:commercial|standard|reagent[\s-]grade|analytical[\s-]grade|hplc[\s-]grade|acs[\s-]grade|technical[\s-]grade)\s+"
    r"|(?:cross[\s-]?linked|modified|functionalized|grafted|activated|treated|coated|doped|supported)\s+"
    r")*",
    re.IGNORECASE,
)

_CORE_STRIP_SUFFIXES = re.compile(
    r"\s*(?:"
    r"solution|solutions|buffer|dispersion|suspension|emulsion|slurry"
    r"|precursor|precursors|additive|additives|filler|fillers"
    r"|membrane|membranes|substrate|substrates|support|supports"
    r"|composite|composites|material|materials|blend|blends"
    r"|layer|layers|coating|coatings|film|films|thin[\s-]film"
    r"|interlayer|interlayers|surface|surfaces|matrix"
    r"|nanocomposite|nanocomposites"
    r"|reagent|reagents"
    r")\s*$",
    re.IGNORECASE,
)

_CORE_STRIP_QUANTITY = re.compile(
    r"\s+(?:\d+(?:\.\d+)?\s*(?:wt|vol|mol|w/w|v/v|w/v|%|g/L|mg/L|mg/mL|ppm|ppb|M|mM|μM|nM)\b.*)",
    re.IGNORECASE,
)

_CORE_STRIP_PARENTHETICAL = re.compile(r"\s*\([^)]*\)\s*$")


def extract_core_chemical_name(name: str) -> str:
    """
    Strip shell words (solution, buffer, precursor, membrane, wt%, etc.) from a
    chemical entity description to expose the core name suitable for PubChem.

    Examples:
      "TMC solution"               → "TMC"
      "m-phenylenediamine solution"→ "m-phenylenediamine"
      "PIP (0.2 wt%)"             → "PIP"
      "cross-linked PVA membrane" → "PVA"
      "NaCl buffer (2000 ppm)"    → "NaCl"
    """
    s = name.strip()
    # remove trailing parenthetical like "(2000 ppm)" or "(0.1 wt%)"
    s = _CORE_STRIP_PARENTHETICAL.sub("", s).strip()
    # remove trailing quantity like "0.2 wt%" or "1000 mg/L"
    s = _CORE_STRIP_QUANTITY.sub("", s).strip()
    # remove trailing shell words
    prev = None
    while prev != s:
        prev = s
        s = _CORE_STRIP_SUFFIXES.sub("", s).strip()
    # remove leading descriptors
    s = _CORE_STRIP_PREFIXES.sub("", s).strip()
    return s if s else name.strip()


# Patterns that mark a name as "a material class, not a pure compound" →
# not worth spending a PubChem call on.
_NOT_WORTH_CHASING = re.compile(
    r"\b(?:"
    r"poly(?:mer|imide|amide|sulfone|ether|ethersulfone|vinylidene|acrylonitrile|carbonate|urethane|propylene|ethylene|styrene)"
    r"|copolymer|terpolymer|blend|nanocomposite|composite"
    r"|metal[\s-]organic\s+framework|mof|zif[\s-]\d+"
    r"|graphene\s+oxide|reduced\s+graphene|go\b"
    r"|carbon\s+nanotube|cnt|mwcnt|swcnt"
    r"|mesoporous\s+silica|mcm[\s-]41|sba[\s-]15|msp"
    r"|halloysite|nanotube|nanosheet|nanofiber|nanofiller|nanoparticle|nano"
    r"|membrane\b|substrate\b|support\b|matrix\b|framework\b"
    r"|commercial\s+\w+|filmtec|espa|bw30|sw30|xle|tfc|ro\s+membrane|nf\s+membrane"
    r"|series\b|model\b|grade\b|product\b"
    r")\b",
    re.IGNORECASE,
)

# Positive indicators that a name IS worth chasing in PubChem.
_WORTH_CHASING = re.compile(
    r"\b(?:"
    r"acid|amine|amide|aniline|imide|chloride|bromide|iodide|fluoride"
    r"|oxide(?!\s+membrane)|hydroxide|sulfonate|sulfate|phosphate|carbonate|nitrate|acetate"
    r"|piperazine|succinimide|pyrrolidone|pyrrolidine|imidazol"
    r"|alcohol|ketone|aldehyde|ester|ether(?!\s+sulfone)"
    r"|benzene|toluene|xylene|phenol|aniline|naphthalene|anthracene"
    r"|methane|ethane|propane|butane|pentane|hexane"
    r"|methanol|ethanol|propanol|butanol|glycol"
    r"|acetone|acetonitrile|dimethyl|diethyl"
    r"|sodium|potassium|lithium|magnesium|calcium|ammonium"
    r"|trimesoyl|phenylenediamine|terephthaloyl|isophthaloyl"
    r")\b",
    re.IGNORECASE,
)


def should_chase_pubchem(name: str, original_abbr: str = "") -> bool:
    """
    Return True if `name` is likely a small molecule / salt / monomer / reagent
    that PubChem can assign a single CID to.

    Return False for:
      - polymer names, composite material classes, MOFs, commercial series
      - names that are too short (<= 1 char) or clearly generic
      - names where KNOWN_ABBR_MAP already maps the abbreviation (those bypass
        this guard and get looked up directly; this function is not called for them)

    The function first checks the extracted core name, then applies blocklist /
    allowlist heuristics.
    """
    if not name:
        return False
    core = extract_core_chemical_name(name)
    if not core or len(core) <= 1:
        return False

    core_l = core.lower()

    # Hard block — always skip
    if _NOT_WORTH_CHASING.search(core):
        return False

    # Hard allow — if the core name contains a clear small-molecule chemical hint
    if _WORTH_CHASING.search(core):
        return True

    # Abbreviations: short ALL-CAPS tokens (2-6 chars) that are not in the
    # blocklist above are potentially worth chasing via KNOWN_ABBR_MAP or DeepSeek
    if re.match(r"^[A-Z][A-Z0-9]{1,5}$", core.strip()):
        return True

    # Generic single words without chemical indicators → not worth chasing
    words = core_l.split()
    if len(words) == 1 and len(core_l) < 4:
        return False

    # If looks_like_single_cid_candidate already passed, trust it
    if looks_like_single_cid_candidate(core, original_abbr):
        return True

    return False


def lookup_known_abbr(name: str) -> str:
    """
    Check if `name` (or its ALL-CAPS form, or a stripped core) is a key in
    KNOWN_ABBR_MAP.  Returns the canonical PubChem query string or "".
    """
    if not name:
        return ""
    # Try exact first
    if name in KNOWN_ABBR_MAP:
        return KNOWN_ABBR_MAP[name]
    upper = name.upper()
    if upper in KNOWN_ABBR_MAP:
        return KNOWN_ABBR_MAP[upper]
    # Also check the core (strip solution/membrane/etc.)
    core = extract_core_chemical_name(name)
    if core != name:
        if core in KNOWN_ABBR_MAP:
            return KNOWN_ABBR_MAP[core]
        core_upper = core.upper()
        if core_upper in KNOWN_ABBR_MAP:
            return KNOWN_ABBR_MAP[core_upper]
    return ""
...
def run_full_enrichment_pipeline(
    ...
    # ── Step 0a: KNOWN_ABBR_MAP fast path ──────────────────────────────────
    # If the original name or its core is a well-known abbreviation, resolve it
    # immediately without spending a DeepSeek call.
    _abbr_resolved = lookup_known_abbr(original_name) or lookup_known_abbr(query)
    if _abbr_resolved:
        base_notes = "; ".join(x for x in [base_notes, f"known_abbr_map: {original_name!r}→{_abbr_resolved!r}"] if x)
        # Run the resolved name through the full PubChem pipeline but skip DeepSeek
        # by treating it as a SINGLE compound.
        _chem, _notes = None, base_notes
        try:
            _chem_r, _notes_r = (
                base_mod.pubchem_lookup(
                    _abbr_resolved, pubchem_cache,
                    api_key=api_key, model=model,
                    rewrite_cache=rewrite_cache, material_cache=material_cache,
                ),
                base_notes,
            )
            _chem_r = dict(_chem_r)
            _chem_r.setdefault("Standardized_Query", _abbr_resolved)
            _chem_r["_PubChem_Query"] = _abbr_resolved
            if hasattr(base_mod, "apply_no_hit_reclassification"):
                _chem_r, _notes_r = base_mod.apply_no_hit_reclassification(
                    chem=_chem_r, query=_abbr_resolved, original_name=_abbr_resolved,
                    record_type="backtrace_known_abbr",
                    api_key=api_key, model=model,
            ...
            _chem, _notes = _chem_r, _notes_r
        except Exception:
            pass
        if _chem and _chem.get("Lookup_Status") in {"OK", "ABBR_EXPANDED_OK", "CID_ONLY"}:
            return _chem, _notes

    # ── Step 0b: extract core chemical name for better PubChem query ────────
    # Strip shell words (solution, membrane, precursor, wt%, etc.) from q_norm
    # to improve PubChem hit rate. The stripped form is used as an *additional*
    # query variant, not as a replacement (we keep q_norm as a fallback).
    _core_name = extract_core_chemical_name(q_norm)
    if _core_name and _core_name.lower() != q_norm.lower():
        # Also check KNOWN_ABBR_MAP for the core form
        _core_abbr = lookup_known_abbr(_core_name)
        if _core_abbr and not _abbr_resolved:
            base_notes = "; ".join(x for x in [base_notes, f"known_abbr_map(core): {_core_name!r}→{_core_abbr!r}"] if x)
            try:
                _chem_c = dict(base_mod.pubchem_lookup(
                    _core_abbr, pubchem_cache,
                    api_key=api_key, model=model,
                    rewrite_cache=rewrite_cache, material_cache=material_cache,
                ))
                _chem_c.setdefault("Standardized_Query", _core_abbr)
                _chem_c["_PubChem_Query"] = _core_abbr
                if hasattr(base_mod, "apply_no_hit_reclassification"):
                    _chem_c, _notes_c = base_mod.apply_no_hit_reclassification(
                        chem=_chem_c, query=_core_abbr, original_name=_core_abbr,
                        record_type="backtrace_known_abbr_core",
                        api_key=api_key, model=model,
                        pubchem_cache=pubchem_cache,
                        rewrite_cache=rewrite_cache,
                        material_cache=material_cache,
                    ...
                    base_notes = _notes_c
                if _chem_c.get("Lookup_Status") in {"OK", "ABBR_EXPANDED_OK", "CID_ONLY"}:
                    return _chem_c, base_notes
            except Exception:
                pass
    def status_score(s):
    ...
    # 注意：call_deepseek_batch 期望的是 original_name（可信化学名称），不是规范化后的 query
    # 这里用 original_name（回溯得到的可信名称）作为分类输入，q_norm 只用于 PubChem 查询
    ds_input = original_name if original_name and original_name.lower() != q_norm.lower() else q_norm
    ds_result = None
    if api_key and hasattr(base_mod, "call_deepseek_batch"):
        try:
            batch = base_mod.call_deepseek_batch([(1, ds_input)], api_key=api_key, model=model)
    ...
    if category in ("POLYMER", "PROCESS", "NON_ENTITY"):
        ...
        # Add core-stripped variant if it differs from q_norm
        if _core_name and _core_name.lower() not in [x.lower() for x in queries_to_try]:
            queries_to_try.append(_core_name)
...
def rank_status(status: str) -> int:
    order = {
        "BACKTRACE_HIT": 8,      # 回溯后 PubChem 命中（最高优先级）
        "BACKTRACE_OK": 7,       # 兼容旧代码
        "BACKTRACE_NAME_OK": 6,
...
def process_one_source(
    ...
    for member, locate_method in candidates:
                    ...
                    if exp_full and exp_type == "SPECIFIC_CHEMICAL":
                        ...
                        if aligned.get("accepted"):
                            ...
                            if query_looks_material(cleaned):
                                ...
                                return current
                            # Gate: only chase PubChem for compounds likely to have a single CID
                            if not should_chase_pubchem(cleaned, original_name):
                                current["Backtrace_Status"] = "BACKTRACE_CLASSIFIED"
                                current["BT_Lookup_Status"] = "NO_SPECIFIC_CHEMICAL"
                                current["BT_Match_Source"] = "MD Backtrace + DeepSeek (material gate)"
                                current["Backtrace_Notes"] = merge_notes(current.get("Backtrace_Notes", ""), f"should_chase_pubchem=False for {cleaned!r}")
                            ...
                            chem, notes1 = try_lookup_query(
                                query=cleaned,
                                # 把回溯得到的可信名称作为 original_name，让 DeepSeek 分类和 apply_no_hit_reclassification 都以此为基准
                            ...
                            if chem.get("Lookup_Status") in {"OK", "ABBR_EXPANDED_OK", "CID_ONLY"}:
                                current["Backtrace_Status"] = "BACKTRACE_HIT"
                                current["BT_Match_Source"] = "MD Backtrace + PubChem"
                                return current
            ...
            if pre_llm:
                ...
                if etype == "specific_chemical" and pre_llm.get("query"):
                    ...
                    if q_norm and is_trustworthy_std_query(q_norm):
                        ...
                        # Gate: only chase PubChem for compounds likely to have a single CID
                        if not should_chase_pubchem(q_norm, original_name):
                            current["Backtrace_Status"] = "BACKTRACE_CLASSIFIED"
                            current["BT_Lookup_Status"] = "NO_SPECIFIC_CHEMICAL"
                            current["BT_Match_Source"] = "MD Backtrace + DeepSeek (material gate)"
                            current["Backtrace_Notes"] = merge_notes(current.get("Backtrace_Notes", ""), f"should_chase_pubchem=False for {q_norm!r}")
                            best = current if rank_status(current.get("Backtrace_Status", "")) > rank_status(best.get("Backtrace_Status", "")) else best
                            continue
                        chem, notes_pre = try_lookup_query(
                            query=q_norm,
                            # 把回溯得到的可信名称作为 original_name
                            original_name=q_norm,
                            base_mod=base_mod,
                            pubchem_cache=pubchem_cache,
                            rewrite_cache=rewrite_cache,
                            material_cache=material_cache,
        ...
        for rc in ranked[:DEFAULT_MAX_REGEX_CANDIDATES]:
            ...
            if not should_chase_pubchem(query, original_name):
                current["Backtrace_Status"] = "BACKTRACE_CLASSIFIED"
                current["BT_Lookup_Status"] = "NO_SPECIFIC_CHEMICAL"
                current["BT_Match_Source"] = "MD Backtrace (material gate)"
                current["Backtrace_Notes"] = merge_notes(current["Backtrace_Notes"], f"should_chase_pubchem=False for {query!r}")
                best = current if best is None or rank_status(current.get("Backtrace_Status", "")) > rank_status(best.get("Backtrace_Status", "")) else best
                continue

            chem, notes1 = try_lookup_query(
                query=query,
                # 把回溯得到的候选名称作为 original_name，让完整流程以它为基准
                original_name=query,
                base_mod=base_mod,
                pubchem_cache=pubchem_cache,
                rewrite_cache=rewrite_cache,
        ...
        if llm_res:
            ...
            if llm_res.get("status") == "SPECIFIC_CHEMICAL" and llm_res.get("query"):
                ...
                if (not aligned.get("accepted")) or query_looks_material(q):
                    ...
                    current["BT_Lookup_Status"] = "NO_SPECIFIC_CHEMICAL"
                    return current
                if not should_chase_pubchem(q, original_name):
                    current["Backtrace_Status"] = "BACKTRACE_CLASSIFIED"
                    current["BT_Lookup_Status"] = "NO_SPECIFIC_CHEMICAL"
                    current["BT_Match_Source"] = "MD Backtrace + LLM (material gate)"
                    current["Backtrace_Notes"] = merge_notes(current["Backtrace_Notes"], f"should_chase_pubchem=False for {q!r}")
                    return current
                chem, notes2 = try_lookup_query(
                    query=q,
                    # 把 LLM 给出的可信名称作为 original_name
                    original_name=q,
                    base_mod=base_mod,
                    pubchem_cache=pubchem_cache,
                    rewrite_cache=rewrite_cache,
...
def process_one_row(
    ...
    for source_idx in tried:
        ...
        if best is None or rank_status(current.get("Backtrace_Status", "")) > rank_status(best.get("Backtrace_Status", "")):
            best = current
        if current.get("Backtrace_Status") in {"BACKTRACE_HIT", "BACKTRACE_OK"}:
            break
...
def merge_backtrace_to_main(entity_df: pd.DataFrame, bt_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Backtrace_Status" in merged.columns:
        ...
        hit_mask = st.isin(["BACKTRACE_HIT", "BACKTRACE_OK"])  # 兼容新旧状态
    ...
    if "Lookup_Status" in merged.columns:
        ...
        if hit_mask.any():
            # BACKTRACE_HIT：回溯后 PubChem 命中，写入 BACKTRACE_HIT 以区分原始 HIT
            bt_hit_mask = st.eq("BACKTRACE_HIT")
            bt_ok_mask = st.eq("BACKTRACE_OK")  # 兼容旧数据
            if bt_hit_mask.any():
                merged.loc[bt_hit_mask, "Lookup_Status"] = "BACKTRACE_HIT"
            if bt_ok_mask.any():
                merged.loc[bt_ok_mask, "Lookup_Status"] = "BACKTRACE_HIT"
        if name_mask.any():
...
# 需要严格限制长度的长文本列（避免单个单元格塞入整段上下文）
_LONG_TEXT_COLUMNS = {
    "Evidence_Snippet", "Context_Window", "Raw_Context",
    "Backtrace_Notes", "Reason", "Context", "Raw_Match_Context",
    "LLM_Explanation", "Resolution_Notes", "Notes"
}


def sanitize_excel_cell(value: Any, max_len: int = 32000) -> Any:
    """
    将单个值清洗为可安全写入 Excel 的内容。

    - 数值/bool 直接返回（保留 Excel 原生类型，不转字符串）
    - 字符串：删除非法控制字符（保留 \\t \\n \\r），去 BOM，去 surrogate，超长截断
    - list/dict/tuple 转 JSON 字符串后同上处理
    """
    if value is None:
        return ""
    # 数值和 bool 直接返回，不做字符串处理
    if isinstance(value, (int, float, bool)):
        return value
    # pd.isna 对非数值类型可能抛异常，单独处理
    try:
        if pd.isna(value):
            return ""
...
def sanitize_df_for_excel(df: pd.DataFrame, max_len: int = 32000, short_text_len: int = 5000) -> pd.DataFrame:
    """
    清洗 DataFrame 所有列，确保可安全写入 Excel。

    - object 列：逐单元格调用 sanitize_excel_cell
    - 数值列（int/float）：保留原始类型，不转字符串
    - 长文本列 (_LONG_TEXT_COLUMNS)：short_text_len 限制
    """
    if df.empty:
        return df.copy()
    ...
    for col in out.columns:
        col_str = str(col)
        is_long_text = any(kw.lower() in col_str.lower() for kw in _LONG_TEXT_COLUMNS)
        limit = short_text_len if is_long_text else max_len

        dtype = out[col].dtype
        if dtype == object:
            # object 列：每个单元格都可能含字符串/list/None，全部清洗
            out[col] = out[col].apply(lambda v, lim=limit: sanitize_excel_cell(v, lim))
        elif pd.api.types.is_string_dtype(dtype):
            out[col] = out[col].apply(lambda v, lim=limit: sanitize_excel_cell(v, lim))
        # int/float/bool 列保留原始类型，openpyxl 可以直接写入
        # 但如果列中混有字符串（混合 object），已在上面处理
    return out


def write_sheet(writer, df: pd.DataFrame, sheet_name: str, max_len: int = 32000, short_text_len: int = 5000) -> None:
    """
    统一写入 sheet 的 helper，自动执行清洗，防止 IllegalCharacterError。
    """
    clean_df = sanitize_df_for_excel(df, max_len=max_len, short_text_len=short_text_len)
    clean_df.to_excel(writer, sheet_name=sheet_name, index=False)


def find_bad_excel_cells(df: pd.DataFrame, max_len: int = 32000) -> pd.DataFrame:
    """
    调试辅助：找出 DataFrame 中包含非法字符或超长文本的单元格。

    返回 DataFrame 包含: column, row_index, value_preview, issue_type
    """
    issues = []
        ...
        for idx, val in df[col].items():
            ...
            if issue:
                ...
                issues.append({
                    "column": col,
                    "row_index": idx,
                    "value_preview": preview.replace("\n", "\\n").replace("\t", "\\t"),
                    "issue_type": issue
                })
    return pd.DataFrame(issues)


def save_bt_only(bt_df: pd.DataFrame, path: str, max_len: int = 32000, short_text_len: int = 5000) -> None:
    """保存 bt_only 文件，自动清洗数据。"""
    out = Path(path)
    out.parent.mkdir(parents=True, exist_ok=True)
    with pd.ExcelWriter(out, engine="openpyxl") as writer:
        write_sheet(writer, bt_df, "bt_all", max_len=max_len, short_text_len=short_text_len)
...
def write_outputs(output: str, merged: pd.DataFrame, bt_df: pd.DataFrame, entity_df: pd.DataFrame,
                  max_len: int = 32000, short_text_len: int = 5000) -> None:
    """写入所有输出 sheet，自动清洗数据防止 IllegalCharacterError。"""
    out = Path(output)
    out.parent.mkdir(parents=True, exist_ok=True)
    ...
    # 调试检查（如需要可取消注释）
    # bad_cells = find_bad_excel_cells(bt_df)
    # if not bad_cells.empty:
    #     print(f"[WARN] 发现 {len(bad_cells)} 个有问题单元格，将被清洗")

    with pd.ExcelWriter(out, engine="openpyxl") as writer:
        write_sheet(writer, final_main_df, "final_main", max_len=max_len, short_text_len=short_text_len)
        write_sheet(writer, review_df, "review_needed", max_len=max_len, short_text_len=short_text_len)
        write_sheet(writer, evidence_df, "evidence", max_len=max_len, short_text_len=short_text_len)
        write_sheet(writer, bt_df, "bt_all", max_len=max_len, short_text_len=short_text_len)

        # abbr_dict_candidates
        dict_cols = [c for c in ["Original_Name", "Resolved_Full_Name", "Resolved_Composition",
                                 "Backtrace_Status", "Backtrace_Confidence", "Evidence_Snippet"]
                     if c in bt_df.columns]
        if dict_cols and "Resolved_Full_Name" in bt_df.columns:
            dict_df = bt_df[bt_df["Resolved_Full_Name"].astype(str).str.len() > 0][dict_cols].drop_duplicates()
            write_sheet(writer, dict_df, "abbr_dict_candidates", max_len=max_len, short_text_len=short_text_len)
...
def main() -> None:
    ...
    summary = {
        ...
        "backtrace_status_counts": bt_df["Backtrace_Status"].value_counts(dropna=False).to_dict() if len(bt_df) else {},
        "resolved_hits": int(bt_df["Backtrace_Status"].isin(["BACKTRACE_HIT", "BACKTRACE_OK"]).sum()) if len(bt_df) else 0,
