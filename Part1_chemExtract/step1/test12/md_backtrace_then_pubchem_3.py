...
# =========================
# 配置区（保留在前面，便于直接改） 对比第二版 加强了回溯部分 使得命中率更高 有LLM
# =========================
...
DEFAULT_OUTPUT_PATH = "./test12/output/testv3_1all.xlsx"  # 输出 Excel 路径
DEFAULT_REPORT_PATH = "./test12/output/testv3_1all.json"  # 输出 JSON 报告路径（可选，不填则不生成）

DEFAULT_BT_INPUT_PATH = "D:/graduate_work/RO_database/Part1_chemExtract/step1/test12/output/testv3_1all_bt_only.xlsx"  # 仅合并模式：已有 bt_only.xlsx 的路径
        ...
        if is_abbr and api_key:
            ...
            if first_occ:
                ...
                if llm_exp and llm_exp.get("entity_type", "NO_CLUE") != "NO_CLUE":
                    ...
                    if exp_type in ("MATERIAL_CLASS", "COMMERCIAL_PRODUCT", "NO_SPECIFIC_CHEMICAL"):
                        best.update({
                    ...
                    if exp_full and exp_type == "SPECIFIC_CHEMICAL":
                        cleaned = cleanup_candidate_name(exp_full)
                        if cleaned and not is_bad_candidate(cleaned):
                    ...
                    best["Backtrace_Notes"] = merge_notes(
                        ...
                        exp_note,
                        "LLM expansion did not pass quality gate; trying regex fallback",
                    )

        # ── Regex 候选抽取（缩写 LLM 失败时的兜底 / 非缩写条目的主路径）──
        regex_candidates = []
        class_only = None
