...
DEFAULT_OUTPUT_PATH = "./test12/output/test7_2.xlsx"  # 输出 Excel 路径
DEFAULT_REPORT_PATH = "./test12/output/test7_2.json"  # 输出 JSON 报告路径（可选，不填则不生成）

DEFAULT_BT_INPUT_PATH = r"D:\graduate_work\RO_database\Part1_chemExtract\step1\test12\output\test7_2_bt_only.xlsx" # 仅合并模式：已有 bt_only.xlsx 的路径
...
def normalize_pubchem_query(query: Any) -> str:
    ...
    if not q:
        return ""
    q = re.sub(r'^[\"\'`]+|[\"\'`]+$', "", q)
    q = re.sub(r"^(and|or|but)\s+", "", q, flags=re.I)
...
def try_lookup_query(
    ...
    def build_variants(q: str) -> List[str]:
        q0 = normalize_pubchem_query(q)
        cands: List[str] = []
        seen = set()
        def add(x: str):
            x = normalize_pubchem_query(x)
            if x and x.lower() not in seen:
                seen.add(x.lower())
                cands.append(x)
        add(q0)
        add(cleanup_candidate_name(q0))
        stripped = re.sub(r"\b(solvent|derivative|membrane|composite|material|monomer|polymer|product|grade)\b$", "", q0, flags=re.I).strip(" ,;:-")
        add(stripped)
        if "," in q0:
            add(q0.split(",", 1)[0].strip())
        if " and " in q0.lower():
            add(re.split(r"\band\b", q0, flags=re.I)[0].strip())
        if "/" in q0 and len(q0.split("/")[0].strip()) >= 4:
            add(q0.split("/")[0].strip())
        add(re.sub(r"\([^)]*$", "", q0).strip(" ,;:-"))
        return [x for x in cands if is_trustworthy_std_query(x)][:6]
    ...
    def status_score(status: str) -> int:
        return {
            ...
            "ABBR_EXPANDED_OK": 6,
            "CID_ONLY": 5,
            "NO_SPECIFIC_CHEMICAL": 2,
            "Polymer": 2,
            "NO_HIT": 1,
            "EMPTY_QUERY": 0,
        }.get(str(status or ""), 0)

    def run_pubchem_once(q: str, record_type: str, notes_in: str) -> Tuple[Dict[str, Any], str]:
    ...
    if not variants:
        q0 = normalize_pubchem_query(query)
        variants = [q0] if q0 else []

    best_chem: Dict[str, Any] = {"Lookup_Status": "EMPTY_QUERY", "Standardized_Query": normalize_pubchem_query(query)}
    best_notes = base_notes
    best_score = -1

    for q in variants:
        q_work = normalize_pubchem_query(q)
        notes_work = base_notes
        llm_category = ""
        llm_components: List[Dict[str, Any]] = []

        # emulate code_extract_name_fixed.py: classify first
        if api_key and hasattr(base_mod, "call_deepseek_batch"):
            try:
                parsed_map = base_mod.call_deepseek_batch([(1, q_work)], api_key=api_key, model=model)
                parsed = parsed_map.get(1, {}) if isinstance(parsed_map, dict) else {}
                llm_category = str(parsed.get("category", "") or "").upper()
                if llm_category:
                    notes_work = merge_notes(notes_work, f"bt_cls={llm_category}")
                if isinstance(parsed.get("components"), list):
                    llm_components = parsed.get("components", [])
                sq = normalize_pubchem_query(parsed.get("single_query", ""))
                if sq and llm_category in {"SINGLE", "SINGLE_NOISY", "POLYMER"}:
                    q_work = sq
            except Exception:
                pass

        # ABBR: try expansion before PubChem
        if llm_category == "ABBR" and api_key and hasattr(base_mod, "resolve_abbr_candidate_once"):
            try:
                ab = base_mod.resolve_abbr_candidate_once(q_work, api_key=api_key, model=model, context_name=original_name)
                if str(ab.get("status", "")).upper() == "CHEMICAL":
                    q_ab = normalize_pubchem_query(ab.get("query", ""))
                    if q_ab:
                        notes_work = merge_notes(notes_work, f"abbr_expanded={q_ab}", ab.get("note", ""))
                        q_work = q_ab
            except Exception:
                pass

        # MIXTURE: split components then try each component
        if llm_category == "MIXTURE":
            comp_queries: List[str] = []
            for c in llm_components[:6]:
                if isinstance(c, dict):
                    n = normalize_pubchem_query(c.get("name", ""))
                    if n:
                        comp_queries.append(n)
            if (not comp_queries) and hasattr(base_mod, "split_mixture_components"):
                try:
                    comps2 = base_mod.split_mixture_components(q_work, {}, set())
                    for c in comps2[:6]:
                        n = normalize_pubchem_query(c.get("name", "")) if isinstance(c, dict) else ""
                        if n:
                            comp_queries.append(n)
                except Exception:
                    pass
            for cq in comp_queries:
                chem, notes = run_pubchem_once(cq, "backtrace_mixture_component", merge_notes(notes_work, f"mixture_comp={cq}"))
                score = status_score(str(chem.get("Lookup_Status", "")))
                if score > best_score:
                    best_score = score
                    best_chem = chem
                    best_notes = notes
                if str(chem.get("Lookup_Status", "")) in {"OK", "ABBR_EXPANDED_OK", "CID_ONLY"}:
                    return chem, notes
            if comp_queries:
                continue

        # material decomposition rescue (single/noisy/polymer or unknown)
        if hasattr(base_mod, "resolve_material_to_pubchem_queries"):
            try:
                cands = base_mod.resolve_material_to_pubchem_queries(q_work, api_key=api_key, model=model, material_cache=material_cache)
                for cq in [normalize_pubchem_query(x) for x in cands[:4] if normalize_pubchem_query(x)]:
                    chem, notes = run_pubchem_once(cq, "backtrace_material_component", merge_notes(notes_work, f"material_decomp={cq}"))
                    score = status_score(str(chem.get("Lookup_Status", "")))
                    if score > best_score:
                        best_score = score
                        best_chem = chem
        ...
        chem, notes = run_pubchem_once(q_work, "backtrace_abbr", notes_work)
        score = status_score(str(chem.get("Lookup_Status", "")))
        if score > best_score:
            best_score = score
            best_chem = chem
...
def process_one_source(
    ...
    for member, locate_method in candidates:
        ...
        # ── 非缩写/重检条目：优先让 LLM 从上下文中判断是否可能翻案成 HIT ──
        regex_candidates = []
        class_only = None
        if (not is_abbr) and enable_llm and api_key and normalize_text(row.get("Lookup_Status", "")).upper() in DEFAULT_RECHECK_STATUSES:
            raw_meta = {"file_name": raw_file, "doi": raw_doi, "title": raw_title}
            pre_llm = call_context_resolver_llm(
                token=original_name,
                windows=windows,
                raw_meta=raw_meta,
                base_mod=base_mod,
            ...
            if pre_llm:
                best["Backtrace_Notes"] = merge_notes(best.get("Backtrace_Notes", ""), f"pre_recheck_llm={pre_llm.get('status','')}")
                if pre_llm.get("status") == "SPECIFIC_CHEMICAL" and pre_llm.get("query"):
                    aligned = align_candidate_for_pubchem(
                        token=original_name,
                        candidate=pre_llm.get("query", ""),
                    ...
                    if aligned.get("accepted"):
                        ...
                        current.update({
                            ...
                            "MD_Locate_Method": locate_method,
                            "MD_Candidate_Tried": md_candidate_tried,
                            "Resolved_Full_Name": aligned.get("clean_name", ""),
                            "BT_Standardized_Query": aligned.get("clean_name", ""),
                            "Resolved_Composition": " | ".join(pre_llm.get("composition", [])),
                            "Backtrace_Confidence": max(pre_llm.get("confidence", 0), aligned.get("confidence", 0)),
                            "Evidence_Snippet": pre_llm.get("evidence", "") or (windows[0][:700] if windows else ""),
                            "Backtrace_Notes": merge_notes("recheck_llm_context", pre_llm.get("note", ""), aligned.get("reason", ""), f"md_locate={locate_method}"),
                        })
                        chem, notes_pre = try_lookup_query(
                            query=aligned.get("clean_name", ""),
                            original_name=original_name,
                            base_mod=base_mod,
                            pubchem_cache=pubchem_cache,
                            rewrite_cache=rewrite_cache,
                ...
                elif pre_llm.get("status") == "MATERIAL_CLASS":
                    best["Backtrace_Status"] = "BACKTRACE_CLASSIFIED"
                    best["BT_Lookup_Status"] = "NO_SPECIFIC_CHEMICAL"
                    best["Resolved_Full_Name"] = pre_llm.get("query", "")
                    best["Resolved_Composition"] = " | ".join(pre_llm.get("composition", []))
                    best["Evidence_Snippet"] = pre_llm.get("evidence", "") or (windows[0][:700] if windows else "")
                    best["Backtrace_Confidence"] = pre_llm.get("confidence", 0)
...
try:
    from openpyxl.cell.cell import ILLEGAL_CHARACTERS_RE as _EXCEL_ILLEGAL_CHARS_RE
except Exception:
    _EXCEL_ILLEGAL_CHARS_RE = re.compile(r"[\000-\010]|[\013-\014]|[\016-\037]")

_EXCEL_MAX_CELL_LENGTH = 32767


def _sanitize_cell_for_excel(v: Any) -> Any:
    """与 openpyxl 的 check_string 一致：去掉非法控制字符并截断长度；数值/日期/布尔原样保留。"""
    if v is None:
        return v
    if pd.isna(v):
        return v
    ...
    if s in ("nan", "NaT", "<NA>"):
        return ""
    s = _EXCEL_ILLEGAL_CHARS_RE.sub("", s)
    return s[:_EXCEL_MAX_CELL_LENGTH]


def sanitize_df_for_excel(df: pd.DataFrame) -> pd.DataFrame:
    """对所有可能以文本写入 Excel 的列做清洗（含 pandas StringDtype，不仅 object）。"""
    out = df.copy()
    for col in out.columns:
        ...
        if pd.api.types.is_categorical_dtype(s):
            s = s.astype("object")
        out[col] = s.map(_sanitize_cell_for_excel)
    return out
...
def main() -> None:
    ...
    if args.process_all_unresolved:
        work_df = work_df[work_df.apply(is_recheck_candidate, axis=1)].copy()
    else:
