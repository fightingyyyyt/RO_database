...
Key ideas
---------
1. Do NOT aggressively strip all parentheses. Internal parentheses are often part of valid IUPAC names.
2. First classify rows: single compound vs mixture/material/process/model code/fragment.
3. Build an abbreviation dictionary from the corpus, e.g.:
      1,4-cyclohexanediamine (CHDA)  -> CHDA => 1,4-cyclohexanediamine
4. Use a layered resolver:
      PubChem compound-name search
      PubChem substance-name search
      OPSIN (name -> SMILES) -> PubChem by SMILES
      CIR / CACTUS (name -> SMILES) -> PubChem by SMILES
5. Use RDKit AFTER a structure is found, for validation/canonicalization.
6. Write a clean step2.xlsx (3 columns) plus detailed debug/failure workbooks.

Recommended install
-------------------
conda install -c conda-forge rdkit openpyxl pandas
pip install pubchempy requests transformers sentencepiece torch py2opsin deep-translator tqdm
"""
...
class ChemicalResolver:
    ...
    def resolve_one(self, raw_name: str) -> ResolveResult:
        ...
        if best is not None:
            ...
            try:
                smiles = (
                    getattr(best, "isomeric_smiles", None)
                    or getattr(best, "connectivity_smiles", None)
                    or None
                )
            except Exception:
                smiles = None
...
def main():
    input_file = "./test3/step1.xlsx"
    output_main = "./test3/step2.xlsx"
    output_debug = "./test3/step2_debug.xlsx"
    output_failed = "./test3/step2_failures.xlsx"
    ...
    print(f"Loaded {len(names)} rows from {input_file}")
    print("Initializing resolver (translator / caches / optional parsers)...")
    ...
    found_n = 0
    failed_or_skipped_n = 0

    print(f"Abbreviation map size: {len(resolver.abbrev_map)}")
    print("Starting resolution...\n")

    iterable = enumerate(names, start=1)
    if HAS_TQDM:
        iterable = tqdm(iterable, total=len(names), desc="Resolving chemicals", unit="item")

    for i, name in iterable:
        ...
        if res.Status == "FOUND":
            ...
            failed_or_skipped_n += 1

        if HAS_TQDM:
            iterable.set_postfix(found=found_n, failed_or_skipped=failed_or_skipped_n, refresh=False)
        elif i % 20 == 0 or i == len(names):
            print(f"[{i}/{len(names)}] found={found_n} failed_or_skipped={failed_or_skipped_n}")
